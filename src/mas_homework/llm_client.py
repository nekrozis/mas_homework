...
class LLMClient:
    async def chat(self, model: str, messages: List[Dict[str, str]], format: Optional[str] = None) -> LLMResponse:
...
class OllamaClient(LLMClient):
    ...
    async def chat(self, model: str, messages: List[Dict[str, str]], format: Optional[str] = None) -> LLMResponse:
        ...
        try:
            ...
            raw_response = await self._client.chat(model=model, messages=messages, options=options)
